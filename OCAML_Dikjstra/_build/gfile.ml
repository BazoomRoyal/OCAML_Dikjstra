...
let export graph path =
 ...
 (* let aff_vertex vertex =
    iter (fun e -> out "%s -> %s [ label = %s ];\n" vertex.id e.id e.e ) vertex.outedges*)
  let aff_vertex = (fun v-> (List.iter (fun (x,y) -> out "%s -> %s [ label = %s ];\n" v.id y x) v.outedges))
  in
  v_iter graph (aff_vertex) ;
