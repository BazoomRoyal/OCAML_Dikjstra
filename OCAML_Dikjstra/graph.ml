...
type ('v, 'e) vertex_info =
  { label: 'v ; (* label du sommet *)
    id: id ;
    outedges: ('e * id) list ;(* 'e = label de l'arete *)
    inedges: ('e * id) list }
...
(* Done. *)
  ...
  (* Même fonctions que le insert/add_edges que ci dessus, avec en plus l'ajout duflow*)
...
let add_edge_addition graph id1 id2 elabel =
  ...
  (* Add (or replace) the given edge. *)
  let new_outedges = insert_and_add [] id2 elabel origin_info.outedges
  and new_inedges  = insert_and_add [] id1 elabel dest_info.inedges in
...
let taille graph = Hashtbl.length graph.vertices
...
let map graph vmap emap =
  ...
  let rec map_edges emap = function
    |[]->[]
    |(x, y)::rest -> ((emap x), y)::map_edges emap rest in


  let map_vertex_info = (fun x -> {label = vmap x.label ;
                                   id = x.id ;
                                   outedges = map_edges emap x.outedges ;
                                   inedges = map_edges emap x.inedges}) in
